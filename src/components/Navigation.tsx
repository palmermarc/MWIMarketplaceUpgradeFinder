...
export function Navigation({ activeTab, onTabChange }: NavigationProps) {
  ...
  return (
    <header className={`${theme.mode === 'classic' ? 'bg-gradient-to-r from-blue-800 via-purple-800 to-indigo-800 border-b border-white/20' : theme.mode === 'dark' ? 'border-b' : `${theme.cardBackground} border-b ${theme.borderColor}`} shadow-lg`} style={theme.mode === 'dark' ? { backgroundColor: '#1a1a1a', borderBottomColor: '#e05a3e' } : {}}>
          ...
          <nav className="flex space-x-1 flex-1 justify-center">
            {tabs.map((tab) => (
              <button
                ...
                style={theme.mode === 'dark' ? { textShadow: '1px 1px 2px rgba(0, 0, 0, 0.8)' } : {}}
              >
                {tab.label}
              </button>
            ))}
