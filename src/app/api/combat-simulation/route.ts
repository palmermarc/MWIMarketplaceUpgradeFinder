...
interface SimulationRequest {
  ...
  targetZone?: string; // Specific zone to test (e.g., "/actions/combat/fly")
  enhancementSlot?: string; // Specific slot to enhance (e.g., "head")
  enhancementLevel?: number; // Enhancement level to test
}

export async function POST(request: NextRequest) {
  try {
    const requestBody = await request.json();
    const { character, rawCharacterData, equipmentOverride, targetZone, enhancementSlot, enhancementLevel }: SimulationRequest = requestBody;

    console.log('API Request Body:', {
      hasCharacter: !!character,
      hasRawCharacterData: !!rawCharacterData,
      hasEquipmentOverride: !!equipmentOverride,
      targetZone,
      enhancementSlot,
      enhancementLevel
    });
    ...
    try {
      ...
      const pasteResult = await page.evaluate((importData) => {
        ...
        if (rawCharacterData) {
          // If we have raw character data, parse it and apply modifications
          try {
            const characterData = JSON.parse(rawCharacterData);

            // Apply equipment overrides if provided
            if (equipmentOverride) {
              Object.entries(equipmentOverride).forEach(([slot, override]) => {
                const equipmentItem = characterData.player?.equipment?.find((item: { itemLocationHrid: string; itemHrid: string; enhancementLevel: number }) =>
                  item.itemLocationHrid === `/item_locations/${slot}`
                );

                if (equipmentItem) {
                  equipmentItem.itemHrid = override.item;
                  equipmentItem.enhancementLevel = override.enhancement;
                } else {
                  // Add new equipment item if it doesn't exist
                  characterData.player.equipment.push({
                    itemLocationHrid: `/item_locations/${slot}`,
                    itemHrid: override.item,
                    enhancementLevel: override.enhancement
                  });
                }
              });
            }

            // Note: Enhancement level testing is now handled by updating the DOM fields directly
            // rather than modifying the character data, so we skip enhancement modification here

            return JSON.stringify(characterData);
          } catch (error) {
            console.error('Failed to apply modifications to raw data:', error);
            return rawCharacterData; // Fallback to original data
          }
        } else {
          // Apply modifications to character object
          ...
          return JSON.stringify(modifiedCharacter);
        }
      })());
      ...
      console.log(`Enhancement Slot: ${enhancementSlot}`);
      console.log(`Enhancement Level: ${enhancementLevel}`);
      // Step 1d.1: Update enhancement level field if testing specific enhancement
      if (enhancementSlot && enhancementLevel !== undefined) {
        console.log(`📈 Updating enhancement level for slot: ${enhancementSlot} to +${enhancementLevel}`);

        const enhancementUpdateResult = await page.evaluate((slot, level) => {
          const enhancementFieldId = `#inputEquipmentEnhancementLevel_${slot}`;
          console.log(enhancementFieldId);
        ...
        console.log(enhancementFieldId);
          ...
          if (enhancementField) {
            ...
            console.log(`✅ Updated ${enhancementFieldId} to ${level}`);
            return { success: true, fieldId: enhancementFieldId, value: level };
          } else {
            console.error(`❌ Enhancement field ${enhancementFieldId} not found`);
            return { success: false, fieldId: enhancementFieldId, error: 'Field not found' };
          }
        }, enhancementSlot, enhancementLevel);

        if (!enhancementUpdateResult.success) {
          throw new Error(`Failed to update enhancement field: ${enhancementUpdateResult.error}`);
        }

        console.log(`✅ Enhancement level updated: ${enhancementUpdateResult.fieldId} = ${enhancementUpdateResult.value}`);
        await new Promise(resolve => setTimeout(resolve, 500)); // Wait for field update to register
      }
      ...
      const modalConfigResult = await page.evaluate((targetZone) => {
        const results = {
        ...
        // Zone selection logic - use specific zone or all zones
        if (targetZone) {
          ...
          if (selectZone) {
            ...
            results.targetZone = targetZone;
            results.message += `Target zone selected: ${targetZone}. `;
          } else {
            results.message += 'Zone select dropdown (#selectZone) not found. ';
          }
        } else {
          // Default to Sim All Zones toggle for baseline analysis
      ...
      console.log('✅ Simulation reached 100% completion!');

      // If testing individual enhancement levels, extract specific values directly
      if (enhancementSlot && enhancementLevel !== undefined) {
        console.log(`Testing enhancement: ${enhancementSlot} +${enhancementLevel}`);

        // Wait a moment for results to stabilize
        await new Promise(resolve => setTimeout(resolve, 2000));

        const enhancementResults = await page.evaluate((enhancementSlot, enhancementLevel) => {
          console.log(`Extracting results for ${enhancementSlot} +${enhancementLevel}`);

          // Extract experience gain from #simulationResultExperienceGain
          ...
          // Extract profit from #profitPreview span
          const profitElement = document.querySelector('#profitPreview span');
          const profitPerHour = profitElement ? parseFloat(profitElement.textContent?.replace(/,/g, '') || '0') : 0;

          console.log(`Extracted - Experience: ${experienceGain}, Profit: ${profitPerHour}`);

          return {
            killsPerHour: 0, // Not used for enhancement testing
            expPerHour: experienceGain,
            profitPerHour: profitPerHour,
            revenuePerHour: 0, // Not used for enhancement testing
            zone: 'Single Zone Test',
            success: true,
            enhancementTest: {
        ...
        await browser.close();
        return NextResponse.json(enhancementResults);
      }

      console.log('Looking for All Zones Data button...');
